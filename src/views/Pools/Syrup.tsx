...
import { useFarms, usePriceCakeUsdt, usePools } from 'state/hooks'
...
const Farm: React.FC = () => {
  ...
  const poolsWithApy = pools.map((pool) => {
    const isEthPool = pool.poolCategory === PoolCategory.BINANCE
    const rewardTokenFarm = farms.find((f) => f.tokenSymbol === pool.tokenName)
    const stakingTokenFarm = farms.find((s) => s.tokenSymbol === pool.stakingTokenName)

    // /!\ Assume that the farm quote price is ETH
    const stakingTokenPriceInETH = isEthPool ? new BigNumber(1) : new BigNumber(stakingTokenFarm?.tokenPriceVsQuote)
